...
class SSHConnection:
    ...
    def __init__(self, vm=None):
        try:
            ...
            self.vminfo = VM.vm_info(self.vm)

            if self.vminfo and len(self.vminfo[0]) == 3:
                # self.ip, self.user, self.password = self.vminfo[0][0], self.vminfo[0][1], self.vminfo[0][2]
                self.ip, self.user, self.password = self.vminfo[0][0], self.vminfo[0][1], \
                                                    keyring.get_password(self.vminfo[0][0], self.vminfo[0][1])
                ...
                logger.error("No vm information found.")
...
def vm_connection_test():
    for vm in VM.objects.all():
        try:
            logger.debug(vm, "connection test")
...
def vm_alive_list():
    results = list()
    for vm in VM.objects.all():
        logger.debug(vm, vm.vm_name, vm.ip)
        if vm.status:
